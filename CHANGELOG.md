...
**[ADR 0015](docs/adr/0015-a-durable-series-stores-what-jira-forgets.md) settles what item 4 stores, and it is much less than the item's name implies.** The obvious reading — keep more sprints than the six `history[]` holds — produces a cache of a system that is still authoritative, and a second copy of an answer disagrees with the first eventually. The record asks one question of every field instead: if we throw this away, can Jira tell us again? Most of a row survives being forgotten. `wipItems` does not, and neither does a forecast that was published. So the store holds the facts Jira stops being able to answer, and nothing else.
