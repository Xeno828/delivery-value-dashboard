...
| `sizeField` | `"app"` | Which field carries an ask's **t-shirt band**, resolved exactly as `askField` is. A band selects which quartile of the board's completed epics an ask is compared against — it is not a number and nothing ranks by it. Absent everywhere is the ordinary case: an ask with no band is sized off the whole reference class. Needs eight completed epics rather than five, because the history is split four ways. [ADR 0029](adr/0029-a-t-shirt-band-selects-a-reference-class.md). |
