...
### What a deploy does not prove, checked from inside the tenant — `make forge-smoke`

`tests/forge_smoke.py` opens the app's project page on the dev site in a real browser, finds the frame the dashboard rendered in, and asserts from inside it: the bridge filled the context bar; the KPI band settled into one state, eight tiles or one refusal, and never a mix; no refusal is set in the note style; the stylesheet and every style attribute survived the iframe's policy; the help marks are named buttons that explain themselves on focus and dismiss on Escape; the health chip is reachable; the landmark, the skip link and the table scopes are there; and the frame's console carries nothing but the inline-style policy reports the page produces by design (counted and printed, never folded into the other errors). It ends with a screenshot in `tests/forge-last-run.png`.

It can do this because it drives the browser rather than running inside the page. The Custom UI iframe is cross-origin — `*.cdn.prod.atlassian-dev.net` under the site — so a browser extension's script cannot reach into it, which is why the Forge route was validated by clicking for its first two weeks. A WebDriver-class tool switches into any frame; Playwright is what the other suites already use, so it is what this one uses, and the frame is located by content (the element with the grid's id) rather than by a hostname that is Forge's to change.

**The sign-in is yours.** The script holds no password. The first run opens a visible browser with its own profile under `.forge-smoke-profile/` and waits up to eight minutes for you to sign in to the site; the profile is git-ignored beside `.env` and `.jira-oauth.json`, because it holds the session cookies, and `tests/security.py` asserts the ignore line by name. Later runs are headless on that session until Atlassian expires it, when the window opens again. Two things learned the first time: an identity provider can finish the sign-in in a tab of its own, so the wait watches every tab; and the first `focus()` into a cross-origin frame can land before the frame is the focused one and fire no focus event, so the check focuses the skip link first.

It is not in `make test`. It needs a deployed environment and a person's session, and a suite that cannot run in CI must not look like one that did. Run it after every `make forge-deploy`; it takes about a minute.
