...
`SERVICE_SHARED_SECRET` is not set in production. The startup guard's own message names the mode it is missing configuration for, so a deploy that sets `SERVICE_AUTH=forge-token` and forgets a value fails to start with a sentence naming the value — which is the behaviour to want and is already tested.
...
**A scheduled weekly rebuild, Mondays 03:00 UTC, that also deploys.**
...
**Trivy in CI on every push and on the weekly rebuild. `HIGH` and `CRITICAL` with a fix available block. `HIGH` and `CRITICAL` with no fix available are reported and do not block. Everything below `HIGH` is reported and does not block.**
...
1. **Build and scan the image locally.** `docs/forge-deployment.md` §3 has the commands. The first local `docker build` is the real test of the `pip install` line added in 1.18.0; CI has built it, this machine has not.
2. **Land the scan policy (§11) and the weekly rebuild (§10) in CI.** Before anything is deployed, so the first deployed image is one the policy has already passed.
3. **Create the project, deploy both regions from the same image**, `SERVICE_AUTH=shared-secret` for now, and confirm `/healthz` and an authenticated `/v1/forecast` over the public URLs. This proves hosting independently of Forge.
