...
- **Move the slice into `agent/tools/`**, have `serve_live.py` import it from there rather than define it, and let `/v1/forecast` accept a context set plus a context id so the tool does the slicing. One implementation, still; both wrappers still delegate and still compute nothing. **This is what was done** — `agent/tools/selection.py`, plus `/v1/forecast-context` and `/v1/slice`.
...
**How the circle was broken.** The resolver must fetch the slice's issues before it can ask for a forecast over them, and the slice is the calculator's decision. So it asks: `/v1/slice` returns the context ids, needs no issues, and costs one round trip. `tests/test_service.py` asserts the route names exactly the contexts the forecast then counts, and that the resolver contains no team comparison of its own — a resolver that filtered by team would be the second implementation, invisibly.
...
6. **The atomic commit — done, 2026-08-25.** `forge lint` reports 0 errors and 0 warnings, and one `MAJOR_VERSION_RULE` approval, which is exactly what converting a `baseUrl` format is supposed to raise. What it was: region-pinned `baseUrl`, the offline refusals removed from the forecast and ask-sequencing resolvers, the corrected manifest comment. Deploy with `--approve MAJOR_VERSION_RULE`, then **uninstall and install** — not upgrade — because the `baseUrl` format change is a major version change and Jira does not widen an existing installation on its own.
7. **Watch the first real token through — done, 2026-08-25.** Accepted, and attributed to `ari:cloud:ecosystem::installation/6cc978ae-…`, which is the installation `forge install list` names. See `docs/forge-deployment.md` §2.

   **What that page load did not prove is the numbers.** The forecast came back in **4 ms**, and a 20,000-trial Monte Carlo does not. Locally the same shape simulates in 74 ms and refuses in 0.1 ms, so 4 ms is the refusal path plus HTTP — almost certainly *"too little completion history to sample from"*, which is what a dev site with nothing finished on it should produce and is [ADR 0007](adr/0007-refuse-rather-than-widen.md) behaving correctly. The pipe is proved end to end; a figure travelling down it is not, and the way to confirm either is to look at the tile. A board with real completed work is what settles it.

8. **Originally: watch the first real token through.** This is the moment §2 of the runbook has been waiting for: `SERVICE_AUTH=forge-token` accepting a token Atlassian minted, with the installation ARI in the log line. Capture the token's real `exp - iat` here.
