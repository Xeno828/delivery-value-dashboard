...
> **Amended 2026-09-04: a page-level statement, and why it is not the banner below.** With every tile saying its own sentence in the same callout, the empty state put ten identical amber blocks in one viewport, and a reader saw wallpaper rather than an answer. The band — the first tile since [ADR 0032](0032-the-band-comes-before-the-verdict.md) — now carries the page-level refusal: its own sentence with the clause, then the cause in one sentence (*no issues match the current filters* / *nothing is loaded yet*) and the one action that changes it, then a note saying that the tiles below speak for themselves and which tiles read the record. The tiles keep their own words. That is the distinction the paragraph below draws: what was ruled out was a banner *instead of* the tiles' sentences, which throws away what the page still knows; a statement *above* them that names the cause preserves it. A modal was considered and rejected — it is the state every new Forge tenant opens in, and a dialog on every first visit is a door in front of the page.
