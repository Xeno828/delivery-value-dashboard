...
So `statusCategory` is absent and the page categorises the raw status name under its own config, as it already does for any file whose producer did not resolve it. `started` is absent for the same reason — recognising the first transition into an "In Progress" status needs that same config — and it is the case that changed once flow boards existed. It used to stop there, and the page printed *"no completed items with both a start and a resolved date"*, which is true and which on a board with no sprints empties the one measure that board most needs. So the resolver sends `statusTransitions`, every move between statuses with the names left undecided, and the page picks the earliest one its own config calls in-progress. The rule still has one implementation and it is still not in a resolver; what changed is that the raw material now travels, because unlike `workingDays` there was nothing already on the wire for the page to derive this from. The burndown series is empty because it is Python that is not running, and the page says *"no burndown series in this dataset"* where the chart would be.
...
The other two are what `forge lint` demands for `GET /board/{id}/sprint/{sid}/issue` — the one call that reads a sprint's issues, and there is no cheaper endpoint: the board-issue endpoint returns what is on the board now rather than what a closed sprint contained, and recovering the sprint from an issue means reading a custom field whose id differs per site. `read:jql:jira` looks broader than the rest and is worth understanding rather than waving through: that agile endpoint is JQL-backed underneath.

That sentence used to end *"this app issues no JQL of its own"*, and since [ADR 0011](0011-a-kanban-context-is-a-window-not-a-clock.md) it does issue some: a flow board has no sprint endpoint to read, so its window is fetched from `/board/{id}/issue` with a membership predicate. The claim that mattered is unchanged and is worth stating exactly. **No text from the page reaches Jira.** The only thing a caller supplies is a context id, and `parseContextId` refuses anything that is not a canonical window token naming one of the three offered lengths — so the set of queries this app can be made to issue is three date pairs per board, each built by the resolver from its own clock. The predicate itself is a pure function in `forge/src/jira.js`, mirrored in `scripts/serve_live.py`, and the two are compared string for string.
