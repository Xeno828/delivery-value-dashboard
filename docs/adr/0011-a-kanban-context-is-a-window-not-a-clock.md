...
**A window is not a clock in the forecaster either, and that took two fixes rather than none.** The Monte Carlo needs no sprint boundary and never has — it samples throughput over a rolling window of days — but the slice assembled around it does. `team_slice()` gathers every context belonging to a team, which on a flow board is three windows of the same board that overlap completely, so every issue arrived up to three times and the forecast came back two and a half times too fast. And the window's `endDate`, which is today, was being used as the default target, so *will this land in time* was answered **0%** against a deadline nobody set. Issues are de-duplicated by key, and a window supplies no default target at all.
