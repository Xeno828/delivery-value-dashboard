...
2. **Offer a window, and be able to load one.** These go together or not at all: a picker entry whose id 404s is worse than a board that is honestly not offered. `sprintsFor()` already knows which boards are flow boards, so `contexts` stops counting them as *"N without sprints and not offered"* and returns a window each; `context` learns to resolve a window id into that board's issues, over both transports. The `contexts` 404 sentence — *"none of them uses sprints … there is nothing to show yet"* — goes at the same time.
3. `contextWorkingDays()` returns `[]` for a window, and `tests/e2e.py` asserts that *Pace vs clock* refuses with the flow-board cause rather than any of the other three.
4. The refusals, one tile at a time, each with the digit-sweep the empty-selection tiles already have.
5. The two silent omissions in the register and the exec card.
