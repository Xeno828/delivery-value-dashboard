...
- **Second surface: the standalone HTML file.** One self-contained file in `dist/`, opened from disk or emailed. It makes zero network calls and uses zero browser storage because the threat model is that it gets forwarded. Live mode adds a local server for the Monte Carlo and brief tiles; without it those two are off by default, one tick away in the tile picker, and say they are not in this copy when shown.
...
- **Demo videos:** `docs/demo.mp4` (about two minutes, captioned, no audio) and `docs/demo-small.mp4`; `docs/demo-script.md` is the captions with their timings. All three, and the screenshots, are produced together by `make demo` from the current build.
- **Screenshots:** `docs/screenshots/` — the dashboard, the context bar, the forecast tile, the wizard's mapping and preview steps; `tests/last-run.png` from the browser suite.
