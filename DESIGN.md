...
Below 1180px every span from 3 to 8 becomes 6 and span 9 becomes 12, so the grid halves rather than promoting wide tiles to full width and stranding their partners. Below 760px everything is span 12 and card tools drop under the title onto their own row; the source badge takes its own line above the toolbar buttons, the Tiles button drops the preset and count it carries, the filter row folds behind one *Filters · n active* line, and the context bar keeps its three pickers and loses the dates and source the title block already states, so the first KPI tile starts inside the first screen. Below 900px the exec summary collapses from a 1.15fr / 2fr pair to one column.
