...
function renderHeader(m) {
  ...
  if (m.health == null) {
    /* Withheld, not floored. A 0/100 here would read as "this sprint is in
       ...
       both are verdicts on a selection nobody has loaded anything into. The
       chip keeps its shape so the header does not jump when data arrives. */
    ...
    hc.dataset.tt = "<b>No sprint health score</b><br>" + esc(m.healthRefusal) +
      "<br><br>Every component of the score is a share of the selected issues, so with none " +
      "selected the score is undefined rather than low. It is withheld rather than shown as a " +
      "number, because a number here reads as a measurement.";
    return;
  }
