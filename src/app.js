...
function applyTiles() {
  ...
  /* The predictability chart and the forecast share a row at 7 + 5. A saved
     copy hides the forecast in every preset, which left the chart alone in an
     eleven-column row — the hole the grid rule forbids at every breakpoint.
     The tile left alone takes the row. */
  const pred = $("#c-pred");
  if (pred) pred.classList.toggle("span-full", on.indexOf("c-pred") >= 0 && on.indexOf("c-forecast") < 0);
