...
function renderExec(m) {
  ...
  // Seven findings can be raised and six fit. Rank them before cutting, so the
  // point that is dropped is the least severe rather than the last one written
  // — and then say it was dropped. An unannounced cut reads as a complete list.
  const sevRank = { critical: 0, serious: 1, warning: 2, info: 3, good: 4 };
  pts.sort((a, b) => sevRank[a.sev] - sevRank[b.sev]);
  const execShown = pts.slice(0, 6), execCut = pts.length - execShown.length;
  $("#exec-list").innerHTML = execShown.map((p, ix) => {
    ...
    (execCut > 0 ? "<li><span aria-hidden=\"true\"></span><span class='note'>Showing the " +
      execShown.length + " most severe of " + pts.length + " findings. The " + execCut +
      (execCut > 1 ? " not shown are the least severe of them" : " not shown is the least severe of them") +
      ", ranked below every point above.</span></li>" : "");
  $("#exec-list").onclick = e => {
  ...
  const ctrl = q === "sequence" ? "" : q === "when"
    ? '<div class="fc-ask"><label for="fc-items">How many items?</label>' +
      ...
      (S.fcItems != null
        ...
        : '<span class="note">Outstanding in the selected sprint. Change it to size any ask.</span>') +
...
function renderRisk(m, items) {
  ...
  // Oldest first, so that when there are more of these than the register can
  // show, the ones that fall off the end are nameable: the least old of them.
  m.critical.filter(i => days(i.created, asOf()) > 14)
    .sort((a, b) => days(b.created, asOf()) - days(a.created, asOf())).forEach(i =>
  ...
  const shown = risks.slice(0, 9), cut = risks.slice(9);
  const cutTally = {};
  cut.forEach(r => { cutTally[r.sev] = (cutTally[r.sev] || 0) + 1; });
  host.innerHTML = shown.map((r, ix) => {
    ...
    // One risk is raised per ageing highest-priority item, so a backlog with a
    // dozen stale criticals overruns the register on its own. Say what fell off
    // the end and why those were the ones: a cut list reads as a whole one.
    (cut.length ? '<div class="note" style="margin-top:6px">Showing the ' + shown.length +
