...
function renderForecast() {
  ...
  if (!S.live) {
    el.innerHTML = '<div class="fc-offline"><b>A forecast needs the live-mode connection.</b>' +
  ...
  const id = S.ctx, fc = S.forecasts[id];
  if (fc === undefined) {
    el.innerHTML = '<div class="note">Running 20,000 simulations…</div>';
    fetchForecast(id);
    return;
  }
  if (fc === null) {
    el.innerHTML = '<div class="fc-refusal"><b>No forecast.</b> The request to the live server failed, ' +
  ...
  if (fc.unknown) {
    el.innerHTML = '<div class="fc-offline"><b>This sprint came with the file, not from the server.</b>' +
  ...
  if (q === "when") {
    ...
    if (!sc.available) { html += fcRefusal(sc); } else {
      ...
      html += '<div class="fc-lead"><b>' + sc.remaining_items + " item" +
        (sc.remaining_items === 1 ? "" : "s") + "</b> outstanding. 85% of simulations finish by <b>" +
    ...
    if (!cap.available) { html += fcRefusal(cap); } else {
      ...
      html += '<div class="fc-lead">By <b>' + esc(fmtD(cap.target_date)) + "</b> — " +
 ...
 *  window drag. */
function fetchForecast(id) {
  if (S.fcPending[id]) return;
  S.fcPending[id] = true;
  fetch("api/forecast?id=" + encodeURIComponent(id), { cache: "no-store" })
    .then(r => (r.ok ? r.json() : (r.status === 404 ? { unknown: true } : null)))
    .then(j => { S.forecasts[id] = j || null; })
    .catch(() => { S.forecasts[id] = null; })
    .then(() => { delete S.fcPending[id]; if (S.ctx === id) renderForecast(); });
}
