...
/** The chip, and the panel behind it.
 *
 *  Hidden when the config covered everything — a permanent notice saying
 *  "nothing was inferred" is furniture, and the footer already names the rules
 *  in force. It appears exactly when there is something a reader would want to
 *  challenge, which is the same test every refusal on this page applies. */
function paintWorkflow() {
  const rows = inferredStatuses(), btn = $("#btn-workflow"), pop = $("#wf-pop");
  if (!btn || !pop) return;
  btn.classList.toggle("hidden", rows.length === 0);
  if (!rows.length) { pop.classList.add("hidden"); return; }
  btn.textContent = "Workflow · " + rows.length + " inferred";
  btn.title = rows.length + " status" + (rows.length === 1 ? "" : "es") +
              " this board uses are not named in the organisation config";
  pop.innerHTML =
    '<h4 id="wf-h">Statuses your config does not name</h4>' +
    "<p class='note'>Every figure on this page depends on which statuses mean " +
    "done and in progress. These were not in the config, so each was read from " +
    "the evidence below. If any is wrong, the numbers above it are wrong.</p>" +
    "<table><thead><tr><th>Status</th><th>Read as</th><th>From</th></tr></thead><tbody>" +
    rows.map(r => "<tr><td>" + esc(r.status) + "</td><td>" + esc(r.readAs) +
                  "</td><td>" + esc(r.from) + "</td></tr>").join("") +
    "</tbody></table>" +
    "<p class='note'>Name each one under <code>statuses.done</code> or " +
    "<code>statuses.inProgress</code> in the organisation config to state it " +
    "rather than infer it.</p>";
}
