...
make test-service  # the routes and the Forge resolver, no browser needed
...
`src/` is four files assembled by `build.py` into a single self-contained HTML file. `agent/tools/` is five dependency-free Python modules — `metrics.py` (facts), `forecast.py` (Monte Carlo on work that exists), `intake.py` (forecasting an ask before any of it exists), `selection.py` (which issues a forecast reads, and what it is told about them), `orgconfig.py` (the per-organisation assumptions the others read out of the dataset). `agent/SKILL.md` is the agent definition; it narrates what the tools return and computes nothing. `scripts/` fetches and generates data. `service/` is one file, `routes.py`: the projection, the caps, the refusals and one function per route ending in `answer(path, body)`, which is what the Forge function runs unchanged under WebAssembly (ADR 0031); it computes nothing of its own. `forge/build-assets.mjs` packs the Python runtime, `agent/tools/` and `routes.py` into a generated, git-ignored `forge/src/assets.js` at deploy, and `forge/src/runtime.js` unpacks and answers. `tests/` is six suites; `tests/test_wasm.py` holds the WebAssembly answer against the native one byte for byte. `forge/` is deployed: the app is registered, the dashboard renders inside the iframe and reads the tenant's own boards over the bridge in `forge/bridge/bridge.js`, and every figure it shows — facts, the trend series and the burndown in the call, the forecast and sequencing as jobs on a consumer function — is computed by that same Python inside the function. There is no remote; the hosted calculator that stood in from 2026-08-25 to 2026-09-03 is retired, and the app is eligible for *Runs on Atlassian*.
...
**Nothing between the tools and a reader may do arithmetic either.** The rule below applies to the agent, and equally to `service/routes.py` and the Forge resolver: they validate, delegate and pass figures through. `tests/test_service.py` asserts every route's answer equals the tool called directly, byte for byte, and `tests/test_wasm.py` asserts the same answer under WebAssembly. A wrapper that computes one percentage is a second implementation, and the day it disagrees, every number in the product becomes something to check rather than read.
...
**A new module type or a scope change is a major version; another entry under a block already declared is not.** Major means `forge deploy` stops for `--approve MAJOR_VERSION_RULE` and the installation then needs `forge install --upgrade`, which Forge itself tells you to run — it is *not* an uninstall, the installation id survives and so does app storage. Minor deploys and upgrades on its own. The distinction is worth knowing before you price a change, because it was written down once as *"declaring a module always is"* — true of the `jira:customField` block itself and of `llm`, false of the second field added under that block, and the over-broad half of it put a wrong cost into an ADR and a changelog entry before a deploy contradicted them. Check it by deploying rather than by reasoning about it; it takes a minute and a stated cost is quotable. The second probe's *"removing a remote is minor"* was true of a bare `baseUrl` and false of one that declared realms — removing that was major, on 2026-09-03. Every one of these is free while the only installation is `development` on the dev site, and that window closes at the first external install. `docs/adr/0025-the-app-declares-a-business-value-field.md` carries the correction. **The manifest declares no remote and must not gain one.** The calculator that stood behind a `remotes` block from 2026-08-25 to 2026-09-03 is retired (ADR 0031); a remote added after the first external install is a major version, a reinstall for every tenant and the *Runs on Atlassian* badge gone, and `tests/test_service.py` asserts that no `remotes` block, no egress permission and no Cloud Run hostname survives under `forge/`.
...
| How the calculator was reached while it existed (superseded) | `docs/adr/0012-the-calculator-is-reached-by-invokeremote.md` |
...
| Why hostnames and realms were chosen once (superseded) | `docs/adr/0030-the-manifest-commits-to-its-hostnames-and-realms-once.md` |
...
| Why the KPI band is the first tile and the verdict the second | `docs/adr/0032-the-band-comes-before-the-verdict.md` |
...
| Where the hosted calculator ran, what it cost, and what is left to delete | `docs/hosting-the-calculator.md` |
