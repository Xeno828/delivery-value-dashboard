...
**A new module type or a scope change is a major version; another entry under a block already declared is not.** Major means `forge deploy` stops for `--approve MAJOR_VERSION_RULE` and the installation then needs `forge install --upgrade`, which Forge itself tells you to run — it is *not* an uninstall, the installation id survives and so does app storage. Minor deploys and upgrades on its own. The distinction is worth knowing before you price a change, because it was written down once as *"declaring a module always is"* — true of the `jira:customField` block itself and of `llm`, false of the second field added under that block, and the over-broad half of it put a wrong cost into an ADR and a changelog entry before a deploy contradicted them. Check it by deploying rather than by reasoning about it; it takes a minute and a stated cost is quotable. Every one of these is free while the only installation is `development` on the dev site, and that window closes at the first external install. `docs/adr/0025-the-app-declares-a-business-value-field.md` carries the correction.
